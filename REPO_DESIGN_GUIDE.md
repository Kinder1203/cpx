...
- `docs/app/`: PRD, functional spec, user flows, wireframes, and design contract for the future app.
...
현재 목표는 해커톤 전 lightweight 세팅이다. 실제 앱은 Streamlit, FastAPI,
Next.js 중 당일 팀 상황에 맞는 하나로 얇게 붙인다. 앱 프레임워크 결정 전에도
환자 카드, 프롬프트, 평가 기준, 검증 하네스는 그대로 재사용되어야 한다.
